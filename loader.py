...
MAX_LENGTH = 256
DATASET_DIR = "data/yelp"
n_cpu = multiprocessing.cpu_count()


def read_data(data_dir, colnames=[LABEL_COL, TEXT_COL]):
    datasets = {}
    for t in ["train", "dev", "test"]:
        df = pd.read_csv(os.path.join(data_dir, f"yelp_{t}.txt"), sep='\t', header=None, names=colnames)
...
if __name__ == "__main__":
    datasets = read_data(DATASET_DIR)
